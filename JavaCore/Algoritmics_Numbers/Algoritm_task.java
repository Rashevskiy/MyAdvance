...
Требования:
•	В классе Algoritm_task должен присутствовать метод getNumbers c одним параметром типа long.
...
*/
public class Algoritm_task {
    {
        System.out.println("This is the Algoritm_task class");
    }

    public static void main(String... args) throws IOException {
        try (
                FileOutputStream outputStream = new FileOutputStream(args[0]);
                InputStream is = Algoritm_task.class.getClassLoader().getResourceAsStream(args[1]);
        ) {
            ;
            ...
            switch (s) {
                case "a": {
                    result = new Algoritm_task().new A();
                    break;
                }
                case "b": {
                    result = new Algoritm_task().new B();
                    break;
                }
                case "c": {
                    result = new Algoritm_task().new C();
                    break;
                }
            }
